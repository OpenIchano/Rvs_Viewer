 ...
 *  @param peerCID   采集端CID
 */
- (void)onRecvStreamerLocationWithLongitude:(double)longitude
 ...
 */
- (void)onRecvCustomData:(NSData *)data
...
/**
 *  获取采集端位置（采集端必须打开定位，否则接收不到定位信息）
 *
 *  @param streamerCID 采集端CID
 */
- (NSInteger)requestStreamerLocationWithCID:(unsigned long long)streamerCID;
 ...
 */
- (NSInteger)sendCustomWithData:(NSData *)data
                     dataDesc:(NSString *)dataDesc
                       toPeer:(unsigned long long)peerCID;
